...
module.exports = async function (source, target, acceptableHost) {
    ...
    if (!mention.hasOwnProperty('author') && jf2.hasOwnProperty('children') && jf2.hasOwnProperty('author')) {
    	mention.author = jf2.author;
    }

	// If author was found and is h-card, done
		// If not an h-card, check if URL and find author on that page using representative h-card parsing
		// if not a URL, use whatever it is as author
	if(mention.hasOwnProperty('author') && !mention.author.hasOwnProperty('type')) {
		if(util.isUrl(mention.author)) {
			// process mention.author for representative h-card
			const authorHtml = await util.fetchHtml(authorUrl);
	        const authorMf2 = parser(authorHtml, authorUrl, 'mf2');
	        const authorCard = hcard(authorMf2, authorURL);
	        if (authorCard) {
	            mention.author = authorCard;
	        }
		}
	}
