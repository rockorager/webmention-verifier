...
module.exports = async function (source, target, acceptableHosts) {
	...
	acceptableHosts = acceptableHosts || [];

	if(acceptableHosts.length === 0) {
		acceptableHosts.push(new URL(target).host);
	}

	if (!acceptableHosts.includes(new URL(target).host)) {
		return {
			statusCode: 400,
	...
	mention["wm-verified"] = new Date().toISOString();
